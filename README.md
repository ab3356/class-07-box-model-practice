# INFO 1300 - Activity - Box Model Practice
...
- note the contents of the project folder

- view `index.html` and `Go Live`

- let's add some spacing for the `blockquote`'s in the `Events` section

- let's add some spacing for the `Advisory` message in the `Welcome` section
  
- let's style a nav bar by incrementally adding these rules:
...
- explore other properties to improve the look of our nav links:
  - round the edges on hover
